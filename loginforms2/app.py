...
def addjobs():
    ...
    if request.method=="POST":
        ...
        cursor.close()
        conn.close()
    return render_template('welcomefact.html',add_jobs=True,name=name,fname=fname,facid=facid)
...
def plcom():
    ...
    if request.method == "POST":
        ...
        cursor.close()
        conn.close()
    return render_template('welcomestudent.html', student=results[0] if results else None, is_plcomt=True)
...
def ds_students():
    ...
    cursor.close()
    return render_template('welcometp.html', ds_students=results[0], dsresultss=dsresultss,is_dstp=True)
